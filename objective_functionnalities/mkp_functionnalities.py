...
beta = 10 ** 20
# Fonction select_problem avec interface utilisateur
def select_problem():
    ...
    # Dropdown pour sélectionner un problème
    selected_problem = st.selectbox("Choisissez un problème parmi la liste :", problems)
    
    if selected_problem:
        try:
            # Convertir le nom en fonction Python
            func = eval(selected_problem)
            st.success(f"Problème sélectionné : {selected_problem}")
        except NameError:
            st.error("Le problème sélectionné n'existe pas.")
            return None, None

        # Déterminer la dimension en fonction du problème
        match selected_problem:
            ...
            case "MKP9" | "MKP10":
                D = 60
            case _:
                st.error("Ce problème n'est pas dans la liste.")
                return None, None

        st.write(f"Dimension du problème : {D}")
        return func, D
    else:
        st.error("Aucun problème sélectionné.")
        return None, None
# Sigmoid activation
def sigmoid(x):
    return 1 / (1 + np.exp(-x))
...
def MKP2(x):
    ...
    z += - beta * np.sum(np.maximum(0, weights_used - ccc).astype(np.float64) )
